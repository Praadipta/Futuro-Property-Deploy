...
class FirecrawlService {
    ...
    async findProperties(city, maxPrice, propertyCategory = "Residential", propertyType = "Flat", limit = 6) {
        try {
            ...
            const urls = [
                `https://www.rumah123.com/jual/cari/?q=${formattedLocation}&limit=${limit}`
            ];
            ...
            const extractResult = await this.firecrawl.extract(
                urls,
                {
                    prompt: `Extract ONLY ${limit} different ${propertyCategory} ${propertyTypePrompt} from ${city} that cost less than ${maxPrice} Billion Rupiah.
                    
                    Requirements:
                    - Property Category: ${propertyCategory} properties only
                    - Property Type: ${propertyTypePrompt} only
                    - Location: ${city}
                    - Maximum Price: ${maxPrice} Billion Rupiah
